...
doge_wts_train <- window(doge_xts, end = c("2021-02-14"))
doge_wts_test <- window(doge_xts, start = c("2021-02-15"))

length(doge_wts_train)
length(doge_wts_test)
```

Let's explore some simple forecasting methods

```{r Simple Forecasting Methods}
mean_doge <- meanf(doge_wts_train, h = 24)
naive_doge <- naive(doge_wts_train, h = 24)
snaive_doge <- snaive(doge_wts_train, h = 24)
drift_doge <- rwf(doge_wts_train, h = 24, drift = TRUE)
