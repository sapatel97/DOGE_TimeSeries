...
```{r Plotting}
autoplot(doge_xts[-1], main = "Dogecoin", xlab = "Time", ylab = "USD")
```
...
Acf(doge_xts) # original
Acf(BoxCox(doge_xts, lambda = "auto")) # transformed
...
```{r}
doge_xts <- diff(doge_xts)
```


```{r Splitting the data}
doge_xts_train <- doge_xts[1:92]
doge_xts_test <- doge_xts[93:116]

doge_ts <- ts(doge_xts)
doge_train <- ts(doge_xts_train)
```
...
arima_ma_2 <- arima(doge_xts_train, order = c(0,0,2))
summary(arima_ma_2)
auto_arima <- auto.arima(doge_xts_train, seasonal = FALSE)
summary(auto_arima)
...
```{r ARIMA Accuracy}
accuracy(fcast_1, doge_xts_test) # MA(3)
accuracy(fcast_2, doge_xts_test) # auto arima
```
